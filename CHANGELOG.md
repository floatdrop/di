...
- **The module path is now `golang.yandex/di`**, and `dihttp` and `dislog`
  move with it. This breaks every caller: change the `go.mod` requirement and
  the imports together, since the old and new paths are two modules as far as
  the toolchain is concerned and a build graph containing both would link two
  copies of the container. No identifier, signature or behaviour changed, so
  the edit is the path and nothing else:
