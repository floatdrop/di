			...
			body: (f) => (
				<>
					<p>
						...
						创建 logger，把它注册成一个服务——这样任何构造函数都能把它当作参数接收——然后检查图并运行。应用本身就是放在它旁边的模块清单：模块就是一个包导出的函数，用来把该包的服务注册到作用域里。后面每一步都会展开其中一个模块，顺序与
						<C>Use</C>应用它们的顺序一致。
					</p>
					{f.main}
					<p>
						这份清单单独放一个文件，因为固定这张图的测试读的是同一份。写在<C>main</C>
						里就会是第二份副本，而只加到其中一份里的模块，会让这个页面上的图描述一个并不会运行的程序。
					</p>
					{f.wiring}
					<p>
						<C>Use</C>
			...
			body: (f) => (
				<>
					<p>
						...
						storage
						的后面，这也正是<C>main</C>列出它们的顺序。在子作用域中注册的包装器只作用于该作用域及其后代。和 storage 一样，这个包也只导出它的
