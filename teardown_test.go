...
// Rollback must wait for Go hooks even when the caller's context is done.
func TestRegressionRollbackAwaitsWorkerHook(t *testing.T) {
	returned := make(chan struct{})
	s := di.New()
	s.Value(&Worker{}).Eager().
		Go(func(ctx context.Context, w *Worker) error {
			<-ctx.Done()
	...
	case <-returned:
	default:
		t.Fatal("rollback returned without awaiting the Go hook")
	}
}
...
func TestStopFromAHookIsReported(t *testing.T) {
	for _, tc := range []struct {
		...
		{"Run", func(s *di.Scope, got chan<- error) {
			s.Value(&DB{}).Eager().
				Go(func(ctx context.Context, _ *DB) error { got <- s.Stop(ctx); <-ctx.Done(); return nil })
		}},
	} {
...
func TestRegressionExpiredStopDoesNotOrphan(t *testing.T) {
	...
	for _, ch := range []chan struct{}{runCancelled, stopped} {
		...
		case <-ch:
		case <-time.After(5 * time.Second):
			t.Fatal("the instance was orphaned: its Go hook or OnStop never ran")
		}
	}
}
